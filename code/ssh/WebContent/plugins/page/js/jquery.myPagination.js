...
(function($) {
		...
		function init(param,obj){
			...
			var defaults = {
              ...
              pageCount: 10,
              pageNumber: 5,
              cssStyle: app_page_skin==null?"msdn":app_page_skin,
              ajax: {
                on: false,
				type : "POST",
			...
			function getPanelTipInfo() {
				...
				if(opts.panel.tipInfo_on){
					var input = "<input type='text' value='" + opts.currPage + "' title='按回车键跳转' >";
			...
			function responseHandle(data){
                ...
                case "json":
					try{
						...
						resultCountRecord = eval("data.countRecord");
					}
                  break;
                case "xml":
